...
import { getExpiringToday } from "@/lib/admin";
...
export default function ExpiringTodayPage() {
  ...
  useEffect(() => {
    getExpiringToday().then((d) => { setData(d); setLoading(false); });
  }, []);

  return (
    <AdminGuard>
      <AdminTable title="Expiring Today" columns={columns} data={data} loading={loading} />
    </AdminGuard>
  );
}
